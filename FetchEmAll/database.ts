...
export async function getAllPokemon():Promise<Pokemon[]> {
    try {
        const allPokemon:Pokemon[] = (await pokedexCollection.find({}).sort({ id : 1 }).toArray()).slice(0, 151);
        return allPokemon;
    } catch (error) {
        console.error(error)
    }
...
export async function getPokemonById(id:number):Promise<Pokemon[]> {
    try {
        const pokemon:Pokemon[] = (await pokedexCollection.find({ id : id }).toArray()).slice(0, 1);
        return pokemon
    } catch (error) {
        console.error(error)
    }
